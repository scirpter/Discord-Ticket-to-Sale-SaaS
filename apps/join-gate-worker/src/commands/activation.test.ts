...
import { JoinGateAccessService, TenantRepository, resetEnvForTests } from '@voodoo/core';
...
describe('join-gate activation command', () => {
  afterEach(() => {
    vi.restoreAllMocks();
    resetEnvForTests();
  ...
  it('blocks non-super-admin users', async () => {
    ...
    expect(reply).toHaveBeenCalledWith({
      content: 'Only the configured super admin Discord ID can manage join-gate activation.',
      flags: MessageFlags.Ephemeral,
    });
  });

  it('grants remote join-gate access by guild ID and user ID', async () => {
    process.env.SUPER_ADMIN_DISCORD_IDS = 'owner-1';
    resetEnvForTests();
    ...
    expect(editReply).toHaveBeenCalledWith({
      content:
        'Granted `/join-gate` access for `234567890123456789` in `Remote Guild` (`123456789012345678`).',
    });
  });
});
