...
export function parseDailyTimeHhMm(timeHhMm: string): { hour: number; minute: number } {
  ...
  if (!match) {
    throw new AppError('SPORTS_SCHEDULE_TIME_INVALID', 'Time must be in HH:mm (24-hour) format.', 422);
  }
...
export function assertValidTimezone(timezone: string): string {
  ...
  if (!trimmed) {
    throw new AppError('SPORTS_SCHEDULE_TIMEZONE_INVALID', 'Timezone is required.', 422);
  }

  try {
    new Intl.DateTimeFormat('en-GB', { timeZone: trimmed }).format();
    return trimmed;
  } catch {
    throw new AppError(
      'SPORTS_SCHEDULE_TIMEZONE_INVALID',
      `Invalid timezone "${timezone}". Use an IANA timezone like Europe/London.`,
      422,
    );
  }
}
